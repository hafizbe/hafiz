...
class Surah < ActiveRecord::Base
  ...
  def get_traduction(lst_traduction, from_verset, to_verset)
    tab_verset = nil
    unless  lst_traduction.nil? or lst_traduction == "none"
      require 'open-uri'
      s3 = AWS::S3.new
      number_chapter = id_surah_to_string(self.id)
      url = s3.buckets['hafizbe'].objects["traduction/#{lst_traduction}/Chapter#{number_chapter}.xml"].url_for(:read, :secure => false).to_s
      document = Nokogiri::XML(open(url))
      versets = document.xpath("//Verse[@VerseID >= #{from_verset} and @VerseID <=#{to_verset}]")
      tab_verset = []
  ...
  def self.load_surahs
    ...
    surahs.each do |surah|
      unless surah.empty?
        ...
        type_surah = surah[7]
        the_surah = Surah.new(:nb_versets => nb_versets, :name_arabic =>name_arabic, :name_phonetic => name_phonetic, :type_surah => type_surah)
        the_surah.save!
        i = i + 1
      end
    end
  ...
  def self.load_verses
    Surah.all.each do |surah|
      for i in 1..surah.nb_versets
        surah.verses.create!(:position =>i, :surah_id => surah.id)
      end
    end
  end
end
